...
def generate(locale: str, name: str, flag: str, idx: int, total: int):
    ...
    section("💳  Finance")
    t = make_table()
    row(t, "IBAN",          sf(fake.iban))
    row(t, "BBAN",          sf(fake.bban))
    row(t, "SWIFT / BIC",   sf(fake.swift))
    row(t, "Card number",   sf(lambda: fake.credit_card_number(card_type=None)))
    row(t, "Card expiry",   sf(fake.credit_card_expire))
    row(t, "CVV",           sf(fake.credit_card_security_code))
    row(t, "Card network",  sf(fake.credit_card_provider))
    row(t, "VAT / Tax ID",  sf(fake.vat_id))
    row(t, "SSN",           sf(fake.ssn))
    console.print(t)

    section("🪪  IDs & Documents")
    t = make_table()
    row(t, "Passport",      sf(fake.passport_number))
    row(t, "License plate", sf(fake.license_plate))
    row(t, "SSN (fmt)",     sf(lambda: fake.numerify("##-##-##-###-###-##")))
    row(t, "UUID",          sf(fake.uuid4))
    row(t, "MD5",           sf(fake.md5))
    row(t, "SHA1",          sf(fake.sha1))
    console.print(t)
